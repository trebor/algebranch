...
- **RadialMenu tour guidance** (2026-06-11): on globalOp steps the radial menu is fully guided. Stage 1: the petal matching the step's op gets the annotation circle (sqrt/root and square/power share petal families); all other petals dim to 35% and lock. Stage 2: the circle sits on the term input / power spinner until the value matches the step's expectation (`tourInputSatisfied`), then jumps to the Apply button; Apply is `disabled` while off-script so the learner can't fire e.g. ×5. The term input's placeholder hints the expected term during the tour. Circle styling factored into `THEME_GLASS.ONBOARDING_CIRCLE` (was hand-copied in 3 places; now used in 6).
