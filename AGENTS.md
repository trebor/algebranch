...
- **What to do, ranking, status, and plans**: GitHub Issues + the **Algebranch Project board** (Project 6, https://github.com/users/trebor/projects/6/views/2). 
...
- **Ranking is manual column order, not a priority field** (since 2026-07-12): the human ranks work by dragging items within a column, so an item's position *is* its priority — topmost first. The old `P0`/`P1`/`P2` Priority field is abandoned; do not set it, and ignore any stale values on old items. `gh project item-list` returns items in this manual order, so the query below needs no sorting — **do not re-sort its output**, that would destroy the signal. To reorder programmatically (only when asked), use the GraphQL mutation `updateProjectV2ItemPosition` (project `PVT_kwHOAMNZWs4BaYSq`; omit `afterId` to place an item at the top, chain `afterId` to lay down a sequence).
- **Picking work**: The ranked queue is the **`Inbox` column** — that's where the human drags items into order. Work the pipeline from the most-advanced column back: resume an `In progress` item if one is yours, else code the topmost `Planned` item (already scoped — don't strand it), else pick up the topmost `Inbox` item and scope it (→ `Planning`). Read the board, in ranked order, with:
