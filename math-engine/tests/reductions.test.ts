...
describe('Algebraic Reducible Options & Labeling Tests', () => {
  ...
  test('should NOT label integer evaluations as "Evaluate to Decimal"', () => {
    ...
    // Standard simplification should still reduce 6 / 2 to 3
    const standardReduction = reductions['rhs'].find(r => r.label === undefined && r.type === 'reduce');
    expect(standardReduction).toBeDefined();
    expect(equationToString(standardReduction!.simplified)).toBe('x = 3');
  });

  test('should offer both constant folding and distribution options for constant fractions', () => {
    const eq = parseEquation('x = (10 - 6) / 2');
    const reductions = getReducibleOptions(eq);

    expect(reductions['rhs']).toBeDefined();
    
    // Constant folding (reduce to 2)
    const reduceOption = reductions['rhs'].find(r => r.type === 'reduce');
    expect(reduceOption).toBeDefined();
    expect(equationToString(reduceOption!.simplified)).toBe('x = 2');

    // Distribution (distribute division)
    const distributeOption = reductions['rhs'].find(r => r.type === 'distribute');
